...
export class MetaInjector {
    ...
    inject(Symbol) {
        ...
        this.container[injectMetadata.type](Symbol.name, async () => {
            ...
            let dependencies        = await Promise.all(dependenciesAliases.map( alias => this.container.make(alias)));
...
export class MetadataInjectorServiceProvider {
    ...
    register() {
        ...
        this.fusion.getByManifest('meta-injector').forEach(Symbol => metaInjector.inject(Symbol, this.container));
    }
}

export function inject(injectType, ...dependencies) {
    return Reflect.metadata('meta-injector', {type: injectType, dependencies: dependencies});
}

export function singleton(...dependencies) {
    return Reflect.metadata('meta-injector', {type: 'singleton', dependencies: dependencies});
}

export function bind(...dependencies) {
    return Reflect.metadata('meta-injector', {type: 'bind', dependencies: dependencies});
}